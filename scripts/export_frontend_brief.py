...
def render(spec: dict, register: dict) -> str:
    ...
    w(f"`{spec['spec_id']}` · **{dc['document_class'].upper()}** · Rev {dc['revision']} · "
      f"issued {dc['issued']} · status **{spec['status']}** · dimensions **mm**")
    w("")
    if dc["document_class"] != "tender":
        w(f"> **This is an {dc['document_class'].upper()}, not a tender.** It exists to shortlist "
          f"designers and cost the work. Commercial terms including IP transfer are "
          f"{'in' if dc.get('commercial_terms_in_scope') else 'OUT of'} scope at this revision.")
    w("")
    ...
    w("## 8. Acceptance matrix")
    w("")
    w("Every requirement, how it is demonstrated, and what counts as a pass.")
    w("A MUST without a pass criterion is an opinion.")
    w("")
    w("| REQ | Stage | Method | Pass criterion |")
    w("|---|---|---|---|")
    for a in spec["acceptance"]:
        w(f"| `{a['requirement_id']}` | {a.get('stage', '—')} | {a['method']} | "
          f"{a['pass_criterion']} |")
    w("")
