...
`scripts/release/classify_candidate_result.py`. Eligibility detection
does not use bare prefix/suffix matching. The existing-tag blocker is
generated by `existing_canonical_tag_blocker()` and recognized only
when:

- the middle token is a canonical `vMAJOR.MINOR.PATCH` tag
- optional evidence `version` / `canonical_tag` fields agree
- `tag_eligibility` is `FAIL`
- every verification field is `PASS`

The only catalogued eligibility kind today is `existing_canonical_tag`.
Other known policy blockers (dirty working tree, missing changelog,
unready changelog, missing git metadata, expected-commit mismatch, the
readiness “canonical tag does not exist” check, tag-inspection errors)
are enumerated as non-eligibility. Unknown blocker wording fails closed
...
extra, unreadable, or identity-disagreeing files fail closed as
verification failure even when `needs.verify.result` is `success`.
