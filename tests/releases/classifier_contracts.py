...
from scripts.release.model import RESULT_BLOCKED, RESULT_FAILED, tag_for_version
...
def assert_verification_classification(item: CandidateClassification) -> None:
    """An evidence defect: FAILED, with something in the failures channel."""
    assert item.kind == CLASS_VERIFICATION
    assert item.result == RESULT_FAILED
    assert item.verification == "FAIL"
    assert item.failures


def assert_policy_blocked_not_eligibility(item: CandidateClassification) -> None:
    """A catalogued policy blocker: BLOCKED, verified, but not an eligibility kind."""
    assert item.kind == CLASS_ELIGIBILITY
    assert item.result == RESULT_BLOCKED
    assert item.verification == "PASS"
    assert item.blockers
    assert item.failures == ()
    assert all(
        eligibility_blocker_kind(blocker, version=item.version) not in ELIGIBILITY_BLOCKER_KINDS
        for blocker in item.blockers
    )


def assert_aggregate_eligibility_only(
    result: str,
    blockers: list[str],
    verification: str,
...
def assert_cli_verification_failure(proc: subprocess.CompletedProcess[str]) -> None:
    ...
    assert cli_field(proc.stdout, "RESULT") == RESULT_FAILED
